...
def Ready_page(request):
    ...
    if cat == 'Portfolio':
        ...
        rdata=Resume.objects.get(Resume_id=idd)
        tid=rdata.Template_id
        temp=Detail.objects.get(id=tid)
        ttype=temp.Temp_type
        data={
                'pid':idd,
        ...
        if ttype == 'Premium':
            if Payment.objects.filter(Resume_id=idd).exists():
                return render(request,'Temp_ready.html',data)
            else:
                Total=temp.Temp_price-(temp.Temp_price*temp.offer()/100)
                Payment.Temp_price = Total*100
...
def success(request):
    ...
    if cat=='Portfolio':
        ...
        pay.Resume_id=pid
        pay.save()
        pdata=Resume.objects.get(Resume_id=pid)
        tid=pdata.Template_id
        temp=Detail.objects.get(id=tid)
        data={
            'pid':pid,
